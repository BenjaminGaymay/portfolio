...
vec4 colors(vec4 p) {
    if (p[0] < 0.)
        p = vec4(0., 0., 0., 0.);
    p = p * 2. + .1;
    return vec4(color, 1.) * p;
}

void main() {
    float noise = fbm(vec4(vPosition * 0.0005, time * 0.01));
    float spots = max(snoise(vec4(vPosition * 0.1, time * 0.01)), 0.);

    gl_FragColor = vec4(noise);
    gl_FragColor *= mix(1., spots, 0.7);
    // gl_FragColor = vec4(spots);

    gl_FragColor = colors(gl_FragColor);
}